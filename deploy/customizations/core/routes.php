...
/**
 * OVERLAY upstream routes.php.
 *
 * Добавлены webhook-роуты для платёжных систем:
 *  - POST /api/web/payment/tbank → App\Controllers\Web\Payments\Tbank
 *    Tinkoff/T-Bank NotificationURL — Tbank долбит при изменении статуса
 *    платежа (Confirmed, Refunded, RebillIdCreated). Без этого endpoint'а
 *    Tbank получал 404 и спамил ретраи 24h.
 *
 * Important: маршрут /payment/* вынесен ВНЕ группы с Cache middleware
 * (cache не нужен для POST webhook, может мешать).
 */
...
$group = $app->group(
    ...
    function (RouteCollectorProxy $group) {
        ...
        // OVERLAY: webhook-эндпоинты платёжных систем ВНЕ группы с Cache middleware.
        // POST /api/web/payment/tbank — Notification URL для Tinkoff/T-Bank.
        $group->post('/web/payment/tbank', App\Controllers\Web\Payments\Tbank::class);
