...
const formRegister = ref({username: '', password: '', fullname: '', email: '', agree: false, captcha_token: ''})
...
async function onRegister() {
  ...
  try {
    ...
    showModal.value = false
    formRegister.value = {username: '', password: '', fullname: '', email: '', agree: false, captcha_token: ''}
    useToastInfo(t('success.register'))
  } catch (e) {
    // токен SmartCaptcha одноразовый — на ошибку (и на success) надо запросить новый,
    // иначе следующая попытка пользователя пошлёт уже использованный токен → 403
