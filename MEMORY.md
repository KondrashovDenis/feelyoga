...
- **Orbita EditorJS блоки: пустая `EDITOR_TOPIC_BLOCKS` = ВСЁ ВЫКЛЮЧЕНО** (counter-intuitive). `forms/topic.vue` делает `editorBlocks = EDITOR_TOPIC_BLOCKS || false` → пустая строка падает в `|| false` → boolean `false` → в `enabledBlocks` уходит `false` → `return props.blocks ? allBlocks : []` → toolbar пустой. Симптом: в редакторе Заметки нет кнопок Header/Image/Video/etc. **Лечение:** в `.env` явно перечислить блоки: `EDITOR_TOPIC_BLOCKS=header,list,image,video,audio,file,embed,code,kbd` (paragraph дефолтный, не в списке). Аналогично `EDITOR_COMMENT_BLOCKS` для комментов.
